...
#takes in a file containing indices of illumina reads
#returns; 
#dictionary for index hop counting {('AGTC','ACGT'),count}
#dictionary for counting correct index matches {'AGTC',count}
#dictionary for getting index name {'GCTCA','J9'}
...
#open dictionary of files. one for each record
...
#divide all index counts by number of records retained
...
#writes out a text file with all counts a d index distribution
...
#main function which iterates through 4 illumina files 
#sorts based on barcode
def get_records(r1_file, r2_file, r3_file, r4_file, index_file, q_cutoff, path_out):
    ...
    #open zipped illumina read files (R1, R2, R3, R4) and iterate through sorting records
    with gzip.open(r1_file,'r') as r1, gzip.open(r2_file,'r') as r2, gzip.open(r3_file,'r') as r3, gzip.open(r4_file,'r') as r4:
        for (r1_line, r2_line, r3_line, r4_line) in zip(r1, r2, r3, r4):
            ...
            cur_record = Record(record_1, record_2, record_3, record_4, q_cutoff)
    
            if cur_record.error == True or cur_record.index_1 not in index_count_dic:
